...
fn test_check_invalid() {
    ...
    assert!(
        !ok && o.contains("AE-EPISTEMIC-001"),
        "invalid should fail: {o}"
    );
}
#[test]
fn test_check_full_pipeline() {
...
#[test]
fn test_run_full_trace() {
    let (ok, o) = run_aethel(&["run", "examples/full_pipeline.aet", "--trace"]);
    assert!(ok && o.contains("Effect Trace"), "trace: {o}");
    assert!(
        o.contains("log_action") && o.contains("execute"),
        "effects: {o}"
    );
}
#[test]
fn test_run_invalid_fails() {
