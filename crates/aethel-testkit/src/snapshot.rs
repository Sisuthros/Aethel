...
pub fn assert_compiles(source: &str) {
    ...
    assert!(!diagnostics.has_errors(), "Parse errors: {:?}", diagnostics.errors());
    
    let (ir, diagnostics) = aethel_check::checker::check_module(&module, file_id);
    assert!(!diagnostics.has_errors(), "Check errors: {:?}", diagnostics.errors());
}

/// Assert that a program fails to compile with specific error.
pub fn assert_compile_error(source: &str, expected_code: aethel_syntax::diagnostic::DiagnosticCode) {
    let file_id = aethel_syntax::span::FileId::new(0);
    let tokens = aethel_syntax::lexer::lex(source, file_id);
    let (module, diagnostics) = aethel_syntax::parser::parse(&tokens, file_id);
